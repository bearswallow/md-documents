...
docker run -d -p 10911:10911 -p 10909:10909 -v /tmwl/rocketmq/broker/logs:/root/logs -v /tmwl/rocketmq/broker/store:/root/store --name rmqbroker --link rmqnamesrv:namesrv -e "NAMESRV_ADDR=namesrv:9876" -e "BROKER_IP1=118.31.55.111" rocketmqinc/rocketmq:4.4.0 sh mqbroker
```
...
```bash
docker run -d --name rmqconsole --restart always -e "JAVA_OPTS=-Drocketmq.namesrv.addr=namesrv:9876 -Dcom.rocketmq.sendMessageWithVIPChannel=false" -p 8080:8080 --linke rmqnamesrv:namesrv styletang/rocketmq-console-ng
```
